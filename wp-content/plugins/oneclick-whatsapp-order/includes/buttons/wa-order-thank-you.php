...
function wa_order_thank_you_override($title, $id)
{
    ...
    $thanks_label = get_option('wa_order_option_thank_you_label', ''); // This variable is fetched but not used in the snippet
    $include_order_number = get_option('wa_order_option_custom_thank_you_order_number', ''); // This variable is fetched but not used in the snippet
    $order_number_label = get_option('wa_order_option_custom_thank_you_order_number_label', ''); // This variable is fetched but not used in the snippet
    ...
    foreach ($order_items as $item_id => $item) {
        ...
        return html_entity_decode($final_output);
    }
}
if ($override_thankyou_page === 'yes') {
    add_filter('woocommerce_thankyou_order_received_text', 'wa_order_thank_you_override', 20, 2);
}
